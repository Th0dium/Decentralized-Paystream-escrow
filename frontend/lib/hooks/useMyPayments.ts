...
import { PAYSTREAM_ABI } from '@/lib/abi'
  ...
  const fetchPayments = useCallback(async () => {
    ...
    try {
      ...
      const [employeeIds, companyIds, auditorIds] = await Promise.all([
        queryStreamIds('employeeStreams', userAddress),
        queryStreamIds('companyStreams', userAddress),
        queryStreamIds('auditorStreams', userAddress),
      ])

      // Fetch full stream details for each ID
      const [employeeStreams, companyStreams, auditorStreams] = await Promise.all([
        fetchStreamsDetails(employeeIds, 'employee', userAddress),
        fetchStreamsDetails(companyIds, 'company', userAddress),
        fetchStreamsDetails(auditorIds, 'auditor', userAddress),
      ])
...
async function queryStreamIds(
  functionName: 'employeeStreams' | 'companyStreams' | 'auditorStreams',
  userAddress: Address
): Promise<number[]> {
  try {
...
async function fetchStreamsDetails(
  streamIds: number[],
  userRole: 'company' | 'employee' | 'auditor',
  userAddress: Address
